...
def dilate_mask_3d(mask: torch.Tensor, padding_size: int) -> torch.Tensor:
    ...
    kernel = torch.ones(
        (kernel_size, kernel_size, kernel_size), dtype=torch.float32, device=mask.device
    )
    kernel = kernel.unsqueeze(0).unsqueeze(0)
    dilated_result = torch.clamp(
        F.conv3d(inverted_mask, kernel, padding=padding_size), 0, 1
    )
...
def temperature_residual(
    ...
    res[:, :, 1:-1, 1:-1, 1:-1] = (
        uc * dTdx + vc * dTdy + wc * dTdz - alpha * lap_T - source_term
    )
    return res
    ...
    for i, data in enumerate(dataset):
        ...
        if plotting and i == 0:
            for chan in range(outvar.size(1)):
                ...
                im = ax[0].imshow(
                    outvar[0, chan, :, :, nz // 2].detach().cpu().numpy(),
                    vmin=vmin,
                    vmax=vmax,
                )
                fig.colorbar(im, ax=ax[0])
                im = ax[1].imshow(
                    pred_outvar[0, chan, :, :, nz // 2].detach().cpu().numpy(),
                    vmin=vmin,
                    vmax=vmax,
                )
...
def main(cfg: DictConfig) -> None:
    ...
    num_ensembles = 5  # Number of ensembles being trained
    base_seed = 1274
    ...
    alpha = cfg.get(
        "alpha", 0.01
    )  # thermal diffusivity used in the temperature equation
    nu = cfg.get("nu", 0.01)
    rho = cfg.get("rho", 1.0)
    ...
    for ensemble_id in range(num_ensembles):
        ...
        checkpoint_dir = f"./checkpoints/ensemble_{ensemble_id}"
        ...
        def _save_loss_curve() -> None:
            ...
            if val_step_log:
                ax.plot(
                    val_step_log, val_losses, label="Val Data", marker="o", markersize=4
                )
            if step_log:
            ...
            with LaunchLogger(
                ...
                for step_in_epoch, data in enumerate(dataset, 1):
                    ...
                    for key, residual in ns_residuals.items():
                        ns_phy_loss = (
                            ns_phy_loss
                            + (mask_phys * residual**2).sum() / ns_mask_denom
                        )
                    ...
                    temp_phy_loss = (
                        mask_phys * temp_res**2
                    ).sum() / mask_phys.sum().clamp_min(1.0)

                    loss = (
                        data_loss + phy_wt * ns_phy_loss + temp_phy_wt * temp_phy_loss
                    )
                    loss.backward()
                    optimizer.step()
                    scheduler.step()
