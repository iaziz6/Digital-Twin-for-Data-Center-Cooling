...
# ===========================
# 3x3 DEEP ENSEMBLE TRAINING FOR ASDL
# - Trains 3 independent baseline UNet models
# - Each model can run for 3 epochs with 100 steps/epoch
# - Uses different random seeds for each ensemble member
# - Saves each model in a separate checkpoint folder
# - Prints every training step for plotting logs
# ===========================
...
def main(cfg: DictConfig) -> None:
    ...
    num_ensembles = cfg.get("num_ensembles", 3)  # 3-model ensemble for ~3-hour budget
    base_seed = 1234
    ...
    # Training data folder on ASDL. Can be overridden with +data_path=...
    path = cfg.get("data_path", "/home/iaziz6/Imo_Training/Training")
    ...
    path_test = cfg.get("val_data_path", path)
    ...
    for ensemble_id in range(num_ensembles):
        torch.manual_seed(
            base_seed + ensemble_id
        )  ## Setting the seed so initialized weights are reproducible
        ...
        # Attempt to load latest checkpoint if one exists for current ensemble
        checkpoint_base_dir = cfg.get("checkpoint_dir", "/home/iaziz6/Imo_Training/Training/checkpoints_ensemble_3x3")
        checkpoint_dir = os.path.join(checkpoint_base_dir, f"ensemble_{ensemble_id}")
        loaded_epoch = load_checkpoint(
            to_absolute_path(checkpoint_dir),
            models=model,
            optimizer=optimizer,
            scheduler=scheduler,
            device=dist.device,
        ...
        log_every = cfg.get("log_every_steps", 1)  # log train loss every N steps
        val_every = cfg.get("val_every_steps", 100)  # run validation every N steps
        save_every = cfg.get("save_every", 1)

        global_step = 0

        logger.info(
            f"Starting ensemble {ensemble_id+1}/{num_ensembles} with "
            f"train_num_samples={cfg.train_num_samples}, val_num_samples={cfg.val_num_samples}, "
            f"max_epochs={cfg.max_epochs}, log_every_steps={log_every}, "
            f"val_every_steps={val_every}, save_every={save_every}, "
        ...
        def _save_loss_curve():
            ...
            ax.set_title(f"Ensemble {ensemble_id} Training Curve")
            ax.legend()
            fig.tight_layout()
            os.makedirs(checkpoint_dir, exist_ok=True)
            fig.savefig(os.path.join(checkpoint_dir, f"loss_curve_ensemble_{ensemble_id}.png"), dpi=120)
            plt.close(fig)
        ...
        for epoch in range(max(1, loaded_epoch + 1), cfg.max_epochs + 1):  # epochs
            with LaunchLogger(
                ...
                for step_in_epoch, data in enumerate(dataset, 1):
                    ...
                    global_step += 1
                    print(
                        f"Ensemble {ensemble_id+1}/{num_ensembles} | "
                        f"Epoch {epoch}/{cfg.max_epochs} | "
                        f"Step {step_in_epoch}/{len(dataset)} | "
                    ...
                    if dist.rank == 0 and global_step % val_every == 0:
                        val_loss = validation_step(
                            ...
                            global_step,
                            plotting=True,
                            name=f"ensemble{ensemble_id}_val_step{global_step}",
                        )
                        _ = validation_step(
                            model,
                            ...
                            global_step,
                            plotting=True,
                            name=f"ensemble{ensemble_id}_train_step{global_step}",
                        )
        ...
        ## Free up memory after each ensemble model is trained
