...
# True MC Dropout requires the same custom UNet class used during train_dropout.py.
# Put unet_mc_dropout.py in the same folder as this script.
try:
    from unet_mc_dropout import UNet
    UNET_SOURCE = "unet_mc_dropout.UNet"
except ModuleNotFoundError as exc:
    raise ModuleNotFoundError(
        "Could not import unet_mc_dropout.py. For true MC Dropout inference, copy "
        "unet_mc_dropout.py into the same folder as this script, or run on ASDL where it exists."
    ) from exc

NX, NY, NZ = 960, 96, 80

DEFAULT_VTU_FILE = "/mnt/c/Users/iaziz6/Downloads/Training/internal_202.vtu"
DEFAULT_MODEL_FILE = "/mnt/c/Users/iaziz6/Downloads/Training/UNet.0.16.mdlus"
DEFAULT_OUTPUT_PREFIX = "internal_202_mc_dropout_epoch16"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run MC Dropout inference on a VTU file and compute RMSE + uncertainty metrics."
    )
    parser.add_argument("--vtu_file", default=DEFAULT_VTU_FILE)
    parser.add_argument("--model_file", default=DEFAULT_MODEL_FILE)
...
def make_positional_encoding(nx=NX, ny=NY, nz=NZ):
    ...
    pos = np.stack(
        [
            xv,
            np.sin(xv * 72 * np.pi / 2),
            np.cos(xv * 72 * np.pi / 2),
            yv,
            np.sin(yv * 8 * np.pi / 2),
            np.cos(yv * 8 * np.pi / 2),
            zv,
...
def build_model(device, model_file):
    if not os.path.exists(model_file):
        raise FileNotFoundError(
            ...
            "Either pass --model_file with the correct path, or copy UNet.0.16.mdlus "
            "into /mnt/c/Users/iaziz6/Downloads/Training/."
        )
...
def enable_mc_dropout(model):
    ...
    for module in model.modules():
        if isinstance(module, (torch.nn.Dropout, torch.nn.Dropout2d, torch.nn.Dropout3d)):
            module.train()
            found_dropout += 1
    print(f"Dropout layers enabled for MC inference: {found_dropout}")
    if found_dropout == 0:
        print("WARNING: No dropout layers found. This will behave like deterministic inference.")
...
def main():
    ...
    metrics_df = pd.DataFrame(
        metrics,
        columns=[
            "Field", "MSE", "RMSE", "MAE", "MaxAbsError",
            "RelRMSE_percent", "Mean_MC_Std", "Max_MC_Std",
        ],
    )
